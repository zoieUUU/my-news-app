...
def get_ranked_news():
    ...
    try:
        ...
        for d in raw_data:
            d['is_s'] = any(stitle.strip() in d['제목'] for stitle in s_titles if len(stitle.strip()) > 5)
    except:
...
with tab1:
    ...
    with l_col:
        ...
        for i, row in enumerate(sorted_list):
            label = f"🔥 [S급 유력] {row['제목']}" if row.get('is_s') else row['제목']
